...
func GetAllUsers(w http.ResponseWriter, r *http.Request) {
	...
	if isValidToken {
		...
		count := 0
		selectCount := "SELECT COUNT(*) FROM USERS where isdeleted=0;"
		rows2, err := d.DB.Query(selectCount)
		if err != nil {
			json.NewEncoder(w).Encode(map[string]interface{}{"Message": err.Error()})
			Logger.Print(err.Error())
			return
		...
		w.WriteHeader(http.StatusOK)
		json.NewEncoder(w).Encode(map[string]interface{}{"Message": "success", "data": list, "pageNo": page, "limit": limit, "totalItems": count})
