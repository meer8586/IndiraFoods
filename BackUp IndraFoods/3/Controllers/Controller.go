...
func VerifyToken(w http.ResponseWriter, r *http.Request) bool {
	...
	// Check for errors while parsing the token
	if err != nil {
		w.WriteHeader(http.StatusUnauthorized)
	...
	// 	w.WriteHeader(http.StatusUnauthorized)
	// 	fmt.Fprint(w, "Invalid token signing method")
	// 	return
	// }

	// Access the claims and perform additional checks
	// if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
	// 	// Extract the desired information from claims
	// 	username := claims["username"].(string)
	// 	role := claims["role"].(string)

	// 	// Perform further checks or operations based on the claims
	// 	// ...
	...
	// token = jwt.NewWithClaims(jwt.SigningMethodHS256, claims)
	// refresh_tokenString, err := token.SignedString(jwtKey)

	// if err != nil {
	// 	w.WriteHeader(http.StatusInternalServerError)
	// 	return false
	// }
	// r.Header.Set("Authorization", "Bearer "+refresh_tokenString)
	return true
}
