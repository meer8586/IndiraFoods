...
func GetReportEmailConfig(w http.ResponseWriter, r *http.Request) {
	...
	if isValidToken {
		...
		rows1, _ := d.DB.Query(getcid, company_id)
		i := 0
		defer rows1.Close()
		for rows1.Next() {
			i++
			rows1.Scan(&company_id)
		}
		if i == 0 {
		...
		count := 0
		selectCount := "SELECT COUNT(*) FROM report_email_config where isdeleted=0 and cid = $1;"
		...
		selectAllReportEmailConfig := "SELECT * from report_email_config where isdeleted=0 and cid = $1 limit $2 offset $3;"
		rows, err := d.DB.Query(selectAllReportEmailConfig, company_id, limit, offset)
		if err != nil {
